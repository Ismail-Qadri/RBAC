...
const GroupModal = ({ groups, roles, onClose, onSave, group }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSave({ ...formData, roleId: parseInt(formData.roleId, 10) });
  };
    ...
    <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-8 transform transition-transform scale-100">
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-2xl font-bold text-gray-800">{group ? t('editGroupModal') : t('addGroupModal')}</h3>
        ...
        <form onSubmit={handleSubmit} dir={language === 'ar' ? 'rtl' : 'ltr'}>
          <div className="mb-4">
            <label className="block text-gray-700 font-semibold mb-2" htmlFor="name">{t('groupName')}</label>
            <input
              type="text"
              id="name"
              name="name"
              ...
              className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500 transition-colors duration-200 ${language === 'ar' ? 'text-right' : ''}`}
              required
            />
          </div>
          <div className="mb-4">
