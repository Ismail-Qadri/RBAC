...
const RoleManagement = ({ roles, resources, permissions, onEdit, onAdd, onDelete }) => {
 ...
 const getUniquePermissionNames = (role, permissions) => {
    ...
    const uniquePermissionIds = [...new Set(role.policies.map(policy => {
    ...
    return uniquePermissionIds.map(permId => {
        ...
        return (
            <span key={permId} className="inline-flex items-center px-2 py-1 mr-2 my-1 bg-white text text-base" title={permName}>
                {permName}
            </span>
        );
    });
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        ...
                        {roles && roles.map((role) => (
                            <tr key={role.id}>
                                ...
                                <td className="px-6 py-4 text-sm text-gray-500">{getUniquePermissionNames(role, permissions)}</td>
